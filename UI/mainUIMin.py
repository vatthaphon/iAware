...
########## DSP
isPlotActivated_g = False
isShutdownDSPThread_g = False
isShutdownComThread_g = False
...
class MainWindow(TemplateBaseClass_g):
    ...
    def __init__(self, refreshRate_p, main_p):
    # Params:
    ...
    # It is guaranteed that this method will be called every 1000/refreshRate_g millisecs.
        ...
        global cmap_g, isPlotActivated_g
    ...
    def run(self):
        global isShutdownDSPThread_g, isShutdownComThread_g, isPlotActivated_g

        isPlotActivated_g = True

    def stop(self):     
        global isPlotActivated_g
    ...
    def exit(self):
        ...
        isPlotActivated_g = False
        isShutdownComThread_g = True
        isShutdownDSPThread_g = True

    # Clean up before exiting.    def closeEvent(self, event):
        global isShutdownDSPThread_g, isShutdownComThread_g, isPlotActivated_g
        ...
        # Part of a project
            ...
            operation_code_l, msg_l = self.__main.signout()
...
def doDSP():
    ...
    global uint16arr_buff_g, is_Padded_g, N_EEG_data_fft_g, isPlotActivated_g

    last_i_EEG_data_fft_l = -1
    i_img_EEG_data_fft_l = 0

    while not isShutdownDSPThread_g:
        
        time.sleep(1.0/img_Fs_fft_g)

        if True and isPlotActivated_g:
...
def communicateWithServer():
    ...
    global isShutdownComThread_g, uint16arr_buff_g
    ...
    try:
        try:
            ...
            while not isShutdownComThread_g:
...
#################### Call from the external program ####################
def run(main_p):
    init()
    ...
    win_g = MainWindow(refreshRate_g, main_p)
