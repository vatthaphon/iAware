...
# from uiv1 import run as mainUIRun
from uiv1 import UI
...
if __name__ == '__main__':    
    ...
    # Child windows
    login_l         = Login(main_p=main_l)
    loginFaceID_l   = LoginFaceID(main_p=main_l)
    signup_l        = Signup(main_p=main_l)

    # mainUI_l        = mainUIRun(main_p=main_l)
    mainUI_l        = UI(refreshRate_p=20, main_p=main_l)        

    signupFaceID_l  = SignupFaceID(main_p=main_l)

    main_l.setChilds(loginFaceID_p=loginFaceID_l, login_p=login_l, signup_p=signup_l, mainUI_p=mainUI_l, signupFaceID_p=signupFaceID_l)

    main_l.setParent(app_p=app_l)

    main_l.hideLoginFaceID()
    main_l.hideSignup()
    main_l.hideMainUI()
    main_l.hideSignupFaceID()
    main_l.showLogin()    
    # login_l.exec_()
    # signup_l.exec_()
